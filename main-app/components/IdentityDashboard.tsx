...
export function IdentityDashboard() {
  ...
  return (
    <div className="w-full space-y-8 animate-in fade-in duration-700">
      {/* Vault Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 border-b border-border/40 pb-6 sm:pb-8">
        <div className="space-y-1">
          <h1 className="text-3xl sm:text-4xl font-black tracking-tight uppercase italic flex items-center gap-3">
            Identity Vault
          </h1>
          <p className="text-muted-foreground font-bold text-[9px] sm:text-[10px] tracking-widest uppercase opacity-50 flex items-center gap-2">
        ...
        {!hasUnlocked && (
          <Button 
            ...
            disabled={isLoading}
            className="w-full sm:w-auto rounded-none h-14 px-8 font-black uppercase tracking-wider text-[10px] sm:text-xs border-2 border-primary bg-primary text-primary-foreground hover:bg-transparent hover:text-primary transition-all group"
          >
            {isLoading ? (
      ...
      <div className="w-full">
        {!hasUnlocked ? (
          <div className="py-20 sm:py-32 flex flex-col items-center text-center space-y-6 bg-muted/5 border-2 border-dashed border-border/20 px-4">
            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-primary/5 flex items-center justify-center text-primary/40">
              <ShieldCheck className="w-8 h-8 sm:w-10 sm:h-10" />
            </div>
            <div className="max-w-md space-y-3">
              <h3 className="text-xs sm:text-sm font-black uppercase tracking-widest leading-tight">Encrypted Metadata Layer</h3>
              <p className="text-[10px] sm:text-xs text-muted-foreground font-medium italic leading-relaxed">
            ...
            <Button variant="outline" size="sm" onClick={handleUnlock} className="rounded-none font-black text-[9px] sm:text-[10px] uppercase tracking-widest px-6 h-10">
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3 border-b border-border/40 pb-4">
              ...
              <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] sm:tracking-[0.3em] text-muted-foreground">Active Subdomain Instances</span>
            </div>

            {identities.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                {identities.map((id) => (
                  <div key={id.id} className="p-5 sm:p-6 bg-muted/10 border border-border/40 hover:border-primary/40 transition-colors space-y-4 group">
                    <div className="flex items-center justify-between">
                      ...
                      <a 
                        ...
                        target="_blank"
                        className="text-muted-foreground/40 hover:text-primary transition-colors p-2 -mr-2"
                      >
                    ...
                    <div>
                      <p className="text-xl sm:text-2xl font-black italic tracking-tighter uppercase group-hover:text-primary transition-colors truncate">
                        {id.id}.zyn.eth
                      </p>
                    ...
                    <div className="pt-4 border-t border-border/20">
                      <p className="text-[8px] sm:text-[9px] font-black uppercase tracking-widest text-muted-foreground/40">Registered Block</p>
                      <p className="text-[10px] sm:text-xs font-mono font-bold">{id.blockNumber}</p>
                    </div>
                  </div>
                ))}
              </div>
